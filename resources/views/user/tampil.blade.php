...
<table class="table table-hover text-center">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                @if($d->role == 'owner')
                    <span class="text-danger fw-semibold">OWNER</span>
                @else
                    <span class="text-primary fw-semibold">ADMIN</span>
                @endif
            </td>
            <td>
                <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal{{ $d->id }}">Edit</button>
                <form action="{{ route('user.destroy', $d->id) }}" class="d-inline-block" method="POST">
                    @csrf
                    @method('delete')
                    <a href="#" onclick="return confirm('Yakin untuk menghapus user?')"> <button class="text-white btn btn-danger">Hapus</button> </a>
                </form>
            </td>
        </tr>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label for="exampleFormControlTextarea1" class="form-label">Role</label>
                        <input type="text" class="form-control text-uppercase" id="exampleFormControlInput1" name="role" readonly value="admin">
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Tutup</button>
